tinymce.PluginManager.add("imageplus", function(a) {
    ...
    function e(d) {
        ...
        if(typeof $.ajaxFileUpload == 'undefined')
            return;
        var body = l.$el.find('#'+ l._id+'-body');
        var fileInput = body.append('<input type="file" name="local_file" id="local_file" style="opacity:0; filter:alpha(0); z-index:-1;">').children('#local_file');
        var uploadBtn = body.find('.mce-combobox').eq(0).append('<div class="mce-btn mce-open mce-abs-layout-item" tabindex="-1" role="button" style="right:1px; top:1px; border:none;"><button type="button" hidefocus="1" tabindex="-1"><i class="mce-ico mce-i-browse"></i></button></div>').find('.mce-open>button');

        function inputChange() {
            //console.log(fileInput[0].id,file.id)
            console.log(fileInput[0] === this)
            if(!/\.(?:jpg|jpeg|png|gif)$/i.test(this.value)) {
                alert('图片格式错误');
                return;
            }
            $.ajaxFileUpload({
                ...
                success: function (data, status) {
                    ...
                    //fileInput.on('change', inputChange);
                },
